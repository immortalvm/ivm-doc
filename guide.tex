...
\item $\MEM{i}$ for $i \in \range{0}{2^N-1}, N \le 64$: Each of these is an 8-bit element.
\end{itemize}
...
\begin{test}
  Verify that your operations for storing and getting the value of an element work for every element, $x$, of sizes 1, 8, and 64, including $\PC$, $\SP$, $\TERM$, and $\MEM{i}$, where $i \in \range{0}{2^N-1}$ and $N \ge 8$.
  \begin{stepnumbers}
...
The entire state of the machine is contained in elements $\PC$, $\SP$, $\TERM$, $\MEM{i}$, and the constant $N$.
...
\item 
  ...
  \begin{tikzpicture}[scale=0.5]
    \foreach \y in {0,...,7}
    {
      \draw (0, 7-\y) node[anchor=east] {$\MEM{\y}$};
