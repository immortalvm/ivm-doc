...
To test \proc{Push} and \proc{Pop}, do as follows, with $N=4$:
\begin{stepnumbers}
\item Set $M[i]$ to 0 for all $i \in \range{0}{2^N-1}$.
...
To test \proc{Fetch}, do as follows, with $N=4$:
\begin{stepnumbers}
\item Set $M[i]$ to 0 for all $i \in \range{0}{2^N-1}$.
\item Initialize the temporary 64-bit element $w$ to 0.
\item Initialize the temporary 64-bit element $x$ to $\hex{FEDCBA9876543210}$.

\item \proc{Put} 8 octets from $x$ into index 0.
\item \proc{Put} 8 octets from $x$ into index 8.

\item \proc{Fetch} 1 octet into $w$.
\item Verify that the value of $w$ is $\hex{0000000000000010}$.
\item Verify that the value of $\PC$ is $\hex{01}$.

\item \proc{Fetch} 2 octets into $w$.
\item Verify that the value of $w$ is $\hex{0000000000005432}$.
\item Verify that the value of $\PC$ is $\hex{03}$.

\item \proc{Fetch} 4 octets into $w$.
\item Verify that the value of $z$ is $\hex{00000000DCBA9876}$.
\item Verify that the value of $\PC$ is $\hex{07}$.

\item \proc{Fetch} 8 octets into $w$.
\item Verify that the value of $w$ is $\hex{DCBA9876543210FE}$.
\item Verify that the value of $\PC$ is $\hex{0F}$.
...
\item \proc{Extend} sign for 1 octet in $y$.
...
\item \proc{Extend} sign for 1 octet in $z$.
...
Every time the machine starts, the memory must be set up first, a program must be loaded into memory, and then the main procedure must be executed.

To set up the memory, set $\TERM$ to 0, set $\PC$ to $0$, and set $\SP$ to $2^N$.

The program is a sequence of $n$ octets which must be stored in $\MEM{i}$ for $i \in \range{0}{n-1}$.
For the purposes of doing the tests below, $\MEM{i}$ for $i \in \range{n}{2^N-1}$ must be set to 0, but this is not normally needed.

When you have set up the memory and stored the program in $\MEM{}$, execute the main procedure repeatedly until the value of $\TERM$ is $\T$.
...
\begin{memtable}
  ...
  \end{memcolumn}
\end{memtable}
When the machine terminates, the value of $\TERM$ should be $\T$, the value of $\PC$ should be $\hex{18}$, the value of $\SP$ should be $\hex{F8}$, and the following elements should have the indicated values.
\begin{memtable}
  \begin{memcolumn}
    $\hex{E8}$ & $\hex{F8}$ \\
    $\hex{E9}$ & $\hex{00}$ \\
