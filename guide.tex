...
\begin{stepnumbers}
\item If the value of $\bitno{x}{8n-1}$ is 1,
  \begin{itemize}[label=]
  \item then for all $i \in \range{8n}{63}$ set $\bitno{x}{i}$ to 1,
...
To initialize the storage, you need to set the elements to predefined values.
...
When you have initialized the storage, simply execute the main procedure repeatedly until the value of $\TERM$ is $\T$.
...
\item\label{itm:main-case} If $k$ is
  ...
  \item[\JUMPZERO{}] then
    ...
    \item \proc{Pop} $x$.
    \item If the value of $x$ is 0,
      \begin{itemize}[label=]
      \item then increment $\PC$ by $a$,
      \item otherwise do nothing.
      \end{itemize}
    \end{stepnumbers}
  \item[\SETSP{}] then
...
To test the machine, first initialize all elements in $\MEM{}$ to the value $\hex{00}$, and then set the following elements to the provided values.
...
\item If $k$ is
  ...
  \item[\ALLOCATE{}] then
    ...
    \item \proc{Pop} $n$.
    \item Create a new range of $n$ new elements in $\MEM{}$.
  ...
  \item[\DEALLOCATE{}] then
    ...
    \item \proc{Pop} $a$.
    \item Discard the range of elements in $\MEM{}$ whose first element is $a$.
    \end{stepnumbers}
  \end{description}
\end{stepnumbers}
