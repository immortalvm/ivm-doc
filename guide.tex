...
The modulo operation $\modulo{x}{y}$ for $y>0$ is defined as follows:
\[ \modulo{x}{y} = x - y \left \lfloor \frac{x}{y} \right \rfloor \]
where
\[ \left \lfloor x \right \rfloor = \{ m \in \mathrm{Z} : m \le x \} \]
and $\mathrm{Z}$ is the set of integers.
...
To test that the storage works as it should, do the following tests several times for $\PC$, $\SP$, $\TERM$, and $\MEM{i}$, where $N=8$ and $i \in \range{0}{2^N-1}$.
...
To test the basic procedures, do as follows, with $N=3$:
\begin{stepnumbers}
...
You first need to obtain the intial program from elsewhere on the film as a sequence of octets.
